...
def merge_attention_dict(attention_dict_lst, config, type_):
  ...
  if type_ == "word":
    for key in dict_keys:
      new_attention_dict[key] = new_attention_dict[key].view(-1, config.max_tweets, config.n_head_word,
...
def check_point_epoch(epoch, model, word_encoder, word_pos_encoder, time_delay_encoder, optimizer, loss,
                      ...
                      acc_test_1, precision_test_1, recall_test_1, f_score_test_1, counter_true_test_1,
                      counter_pred_test_1,
                      acc_test_2, precision_test_2, recall_test_2, f_score_test_2, counter_true_test_2,
                      counter_pred_test_2,
...
def save_best_model(path, model, word_encoder, word_pos_encoder, time_delay_encoder, optimizer, type_, file):
  ...
  path_time_delay_encoder = os.path.join(path, "best_model",
                                         "best_model_time_delay_encoder_" + type_ + "_" + file + ".pt")
