...
class Attention(nn.Module):
  ...
  def forward(self, query, key, val, key_structure=None, val_structure=None, attention_mask=None):
    # q,k,v:（Batchsize,n_head,lenseq,d_model/n_head）
    #key_structure,val_structure: batchsize x n_heads x post x post x d_model/n_head
    ...
    attention_values = torch.matmul(query, key.transpose(len(key.shape) - 2, len(key.shape) - 1))  # QK' transpose是交换最后两个维度
    ...
    if key_structure is not None:
      #(1 x d_model/n_head) * (d_model/n_head x post_num)
      edge_score = torch.matmul(query.unsqueeze(3), key_structure.transpose(4, 3)).squeeze(3)
      #edge_score : batchsize x n_head x post_num x post_num
