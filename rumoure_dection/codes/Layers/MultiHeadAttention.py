...
from rumoure_dection.codes.Layers import Attention
import sys
...
class MultiHeadAttention(nn.Module):
  ...
  def forward(self, query, key, val, key_structure=None, val_structure=None, attention_mask=None):
    #(batchsize,len,d_module) len是有几个词，d_module是词向量的维度
    """
    ...
    residual = query#用来做残差连接的

    # <--------- Getting the projections for each head -------------->

    if self.config.gpu == True:
      # 产生出来的维度为 （n_head,batchsize,len,dim/n_head）
    ...
    for i in range(self.n_head):
      #query为(batchsize,len,dim)  经过Linear model变为(batchsize,len,dim/n_head) ,再在最前面加一个一维，赋给query_head[i]
    ...
    #变为(batchsize,n_head,len,dim/n_head)
    ...
    if key_structure is not None and val_structure is not None:
      ...
      self_atten_features, atten_values = self.attention(query_head, key_head, val_head, key_structure=key_structure,
                                                         val_structure=val_structure, attention_mask=attention_mask)
    ...
    #permute交换维度 (batchsize,numhead,len,dim/numhead)->(batchsize,len,numhead,dim/numhead)
    self_atten_features = self_atten_features.view(*(self_atten_features.shape[:-2]), -1)
    #view将参数重新组合成这个维度 结果为(batchsize,len,dim)
