...
from rumoure_dection.codes.Encoder import LearnedPositionEncoder
...
class PostModule(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.transformer_post = Transformer.Transformer(self.config, self.config.n_mha_layers, self.config.d_model,
                                                    self.config.n_head)

    # <----------- Embedding the posts through n FC layers (post level) ----------->
    if self.config.ff_post:
      self.emb_layer_post = nn.ModuleList(
  ...
  def forward(self, X_word, time_delay, batch_size, num_posts, emb_dim, structure=None, attention_mask=None):
    ...
    if self.config.include_key_structure:
      #structure: batchsize_num_postsx num_posts
    ...
    self_atten_output_post, self_atten_weights_dict_post = self.transformer_post(query_post, key_post, val_post,
    ...
    if self.config.post_module_version == 3:
      ...
      posts_attention_values = self.condense_layer_post(self_atten_output_post)#batch_size*numpost*dim -> batsize*numpost*1
      posts_attention_weights = F.softmax(posts_attention_values.permute(0, 2, 1) + attention_mask.unsqueeze(-2),
                                          dim=-1)
      #attention_wights: batchsize*1*numpost
      del posts_attention_values
      torch.cuda.empty_cache()
      self_atten_output_post = torch.matmul(posts_attention_weights, self_atten_output_post).squeeze(1)
      #self_atten_output_post : batchsize*dim  1维的被压缩了
