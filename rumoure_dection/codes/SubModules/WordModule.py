...
class WordModule(nn.Module):
  ...
  def __init__(self, config):
    ...
    if config.word_module_version in [2, 3, 4]:
      ...
      self.transformer_word = Transformer.Transformer(self.config, self.config.n_mha_layers_word, self.config.d_model,
                                                      self.config.n_head_word)

    # <----------- Embedding the words through n FC layers (word level) ----------->
    if self.config.ff_word:
      self.emb_layer_word = nn.ModuleList(
        [nn.Linear(self.config.emb_dim, self.config.emb_dim) for _ in range(self.config.num_emb_layers_word)])
    ...
    self.dropout = nn.Dropout(p=self.config.dropout_rate, inplace=False)
  ...
  def forward(self, X, word_pos, attention_mask=None):
    ...
    if self.config.word_module_version in [2, 3, 4]:
      ...
      X_word, self_atten_weights_dict_word = self.transformer_word(query_word, key_word, val_word,
    ...
    if self.config.word_module_version == 0:
      ...
      X_word = F.adaptive_max_pool1d(X_word, 1).squeeze(-1)#维度变为 (batch*num_post,embdim,1) squeeze将最后的1维去掉
    ...
    if self.config.word_module_version == 4:
      ...
      #X_word: batch_size x num_posts_x num_words x d_model
      words_attention_values = self.condense_layer_word(X_word)
      #words_attention_values: batch_size x num_posts_x num_words x 1
      words_attention_weights = F.softmax(words_attention_values.permute(0, 1, 3, 2) + attention_mask.unsqueeze(-2),
                                          dim=-1)
